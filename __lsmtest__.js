...
const shouldGetDataFromMemTableAfterDrainEvent = () => {
  return new Promise(async (resolve) => {
    ...
    assert.equal(lsm._ssTablesListBucket.length, 1);
...
const shouldGetDataFromSSTableSegmentFile = () => {
  return new Promise(async (resolve) => {
    ...
    assert.equal(lsm._ssTablesListBucket.length, 2);
...
const checkCompaction = async () => {
  return new Promise(async (resolve) => {
    ...
    assert.equal(lsm._ssTablesListBucket.length, 3);

    await lsm._doCompaction();

    assert.equal(lsm._ssTablesListBucket.length, 5);
...
const dryRun = async () => {
  return new Promise(async (resolve) => {
    const N = 10;
    const MEMTABLE_DRAIN_THRESHOLD = 2;
    const COMPACTION_THRESHOLD = 2;
    const lsm = new LSM(
      basePath,
      1000,
